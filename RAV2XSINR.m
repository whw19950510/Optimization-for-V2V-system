...
%% 图形自变量
powerindex=1;
gamma0=[0,4,8,12,16,20];
gamma0dset=10.^(gamma0/20);
Pcmaxset=[10^(17/10)/1000,10^(23/10)/1000];
round=length(gamma0dset);
sumcapacity=zeros(2,round);
for powerindex=1:2
for epch=1:round
gamma0d=gamma0dset(epch);
Pcmax=Pcmaxset(powerindex);
%% pair matching
Pmc=zeros(M,K);
Pkd=zeros(M,K);
Pkdmin=zeros(1,K);
...
sumcapacity(powerindex,epch)=sum(sum(optimalsumcapacity));
end
end
figure(2)
plot(gamma0,sumcapacity(1,:),'-xb','Linewidth',2);
hold on
plot(gamma0,sumcapacity(2,:),'-or','Linewidth',2);
xlabel('DUE SINR门限gamma0d/dB');
ylabel('遍历容量和/bps/Hz');
grid on
legend('Pcmax=17dbm','Pcmax=23dbm');